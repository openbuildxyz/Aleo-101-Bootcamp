function onIdle(me, enemy, game) {
  ...
  // ========== v7: 全局状态持久化（用于记录敌人最后已知位置） ==========
  // 使用 Function constructor 获取全局对象，兼容各种运行环境
  ...
  function enemyMightBeAimingFromGrass() {
    if (enemyTank) return false;
    if (!aiState.lastKnownEnemyPos) return false;
    // 如果敌人消失不久（30帧内），且最后位置与我同轴，可能正在瞄准
  ...
  function isCentralDangerZone(x, y) {
    ...
    // 检查是否在狭窄通道中
    if (passableNeighbors(x, y) <= 2) {
      // 如果在中央附近且通道狭窄
      if (distToCenter <= 5) return true;
    }
    return false;
  }
  ...
  // ========== v7: 增强子弹检测 - 即使看不到敌人，看到子弹也要躲避 ==========
  function anyBulletThreat() {
    if (enemyBullet && bulletWillHitMe(enemyBullet)) return true;
    // 即使没有直接命中，如果子弹很近也要警惕
    if (enemyBullet && bulletDist(enemyBullet) <= 3) return true;
    return false;
  }
  ...
  function isInEnemyLineOfFire() {
    ...
    // 敌人不可见时，基于最后已知位置判断
    if (enemyMightBeAimingFromGrass()) return true;
    return false;
  }

  // ========== v7: 检查任意位置是否在敌人射击线上（支持最后已知位置） ==========
  ...
  function predictEnemyPos() {
    ...
    // 基于最后已知位置预测
    if (!aiState.lastKnownEnemyPos || !aiState.lastKnownEnemyDir) return null;
    var lx = aiState.lastKnownEnemyPos[0], ly = aiState.lastKnownEnemyPos[1];
    var lDir = aiState.lastKnownEnemyDir;
    if (lDir && DIR_V[lDir]) {
      var plx = lx + DIR_V[lDir][0];
      var ply = ly + DIR_V[lDir][1];
      if (isPassable(plx, ply)) return [plx, ply];
    }
    return aiState.lastKnownEnemyPos;
  }
  ...
  function getSafetyScore(x, y) {
    ...
    if (ePos) {
      score += manhattan(x, y, ePos[0], ePos[1]) * 2;
    }
  ...
  function getEscapeDir() {
    ...
    for (var i = 0; i < DIR_NAME.length; i++) {
      ...
      // 远离敌人（使用最后已知位置）
      if (ePos) {
        var oldDist = manhattan(mx, my, ePos[0], ePos[1]);
        var newDist = manhattan(nx, ny, ePos[0], ePos[1]);
  ...
  function alignedWithEnemy() {
    if (!enemyTank) return false;
    var ex = enemyTank.position[0], ey = enemyTank.position[1];
    return hasClearShot(mx, my, ex, ey);
  }
  ...
  function isStarSafeToGet() {
    ...
    // v7: 敌人消失时更谨慎
    if (!enemyTank && enemyDisappearedNearGrass()) return false;
    return true;
  }
  ...
  function shouldUseCloak() {
    ...
    // v7: 情况6：敌人消失在草地附近且我在开阔地带
    if (!enemyTank && enemyDisappearedNearGrass() && dist <= 5) {
      return true;
    }
  ...
  // ========== v7: 当敌人在附近且没有子弹时，也要保持安全距离 ==========
  function shouldKeepSafeDistance() {
    if (!enemyTank) {
      // v7: 敌人消失时，如果在草地附近也保持安全
      if (enemyDisappearedNearGrass()) return true;
      return false;
    }
  ...
  function getConservativeMove() {
    ...
    for (var i = 0; i < DIR_NAME.length; i++) {
      ...
      // 避免中央危险区域
      if (mapInfo.isArenaLike && isCentralDangerZone(nx, ny)) score -= 40;
  ...
  if (!enemyTank) {
    ...
    if (star && isStarSafeToGet()) {
      ...
      if (moveDir) {
        ...
        // 确保不走进危险区域
        if (!isCentralDangerZone(nx, ny) && !willBeInLineOfFire(nx, ny)) {
  ...
  // v7: 基于最后已知位置的射击线威胁（敌人消失时）
  if (!enemyTank && enemyMightBeAimingFromGrass()) {
  ...
  if (target) {
    ...
    if (moveDir) {
      ...
      if (isGrass(nx2, ny2)) nx2Safe = false;
      if (nx2Safe) {
        if (facing(moveDir)) { me.go(); return; }
        me.turn(moveDir); return;
      }
  ...
  for (var i = 0; i < DIR_NAME.length; i++) {
    ...
    // v7: 避免草地
    if (isGrass(nx, ny)) score -= 25;
    var ePos2 = getEnemyPos();
    if (isAtDisadvantage() && ePos2) {
      score += manhattan(nx, ny, ePos2[0], ePos2[1]);
    }
    // v7: 敌人消失时更保守
    if (!enemyTank && ePos2) {
      score += manhattan(nx, ny, ePos2[0], ePos2[1]) * 2;
    }
