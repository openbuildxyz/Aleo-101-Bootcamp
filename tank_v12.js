...
function onIdle(me, enemy, game) {
  ...
  function isArenaMap() {
    ...
    for (var dx = -1; dx <= 1; dx++) {
      for (var dy = -1; dy <= 1; dy++) {
        ...
        if (nx >= 0 && nx < H && ny >= 0 && ny < W && map[nx][ny] === "x") centerWalls++;
      }
    }
  ...
  // v12: 修复BFS队列限制逻辑 - 使用while+头指针替代for循环
  ...
  if (target) {
    ...
    if (target === star || (star && target && target[0] === star[0] && target[1] === star[1])) {
